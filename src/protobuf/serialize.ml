...
let field_of_enum ~f v = field_of_uint64 (f v)
...
let rec serialize : type a. Writer.t -> (a, Writer.t) protobuf_type_list -> a =
  ...
  let write_packed_field writer ~index ~f rest vs =
    ...
    List.iter ~f:(fun v -> Writer.add_field value_writer (f v)) vs;
  ...
  fun writer -> function
    | Nil -> writer
    | Cons ((index, Double), rest) -> write_field writer ~index ~f:field_of_double rest
    | Cons ((index, Float), rest) -> write_field writer ~index ~f:field_of_float rest
    | Cons ((index, Int64), rest) -> write_field writer ~index ~f:field_of_int64 rest
    | Cons ((index, UInt64), rest) -> write_field writer ~index ~f:field_of_uint64 rest
    | Cons ((index, SInt64), rest) -> write_field writer ~index ~f:field_of_sint64 rest
    | Cons ((index, Int32), rest) -> write_field writer ~index ~f:field_of_int32 rest
    | Cons ((index, UInt32), rest) -> write_field writer ~index ~f:field_of_uint32 rest
    | Cons ((index, SInt32), rest) -> write_field writer ~index ~f:field_of_sint32 rest
    | Cons ((index, Fixed32), rest) -> write_field writer ~index ~f:field_of_fixed32 rest
    | Cons ((index, Fixed64), rest) -> write_field writer ~index ~f:field_of_fixed64 rest
    ...
    | Cons ((index, Bool), rest) -> write_field writer ~index ~f:field_of_bool rest
    | Cons ((index, String), rest) -> write_field writer ~index ~f:field_of_string rest
    | Cons ((index, Bytes), rest) -> write_field writer ~index ~f:field_of_bytes rest
    ...
    | Cons ((index, RepeatedMessage to_writer), rest) ->
      fun vs ->
        let writer =
          List.fold_left
            ~init:writer
            ~f:(fun writer v -> write_message writer ~index ~f:to_writer Nil (Some v))
            vs
        in
        serialize writer rest
    | Cons ((_, Repeated (Message _)), _) -> failwith "Illegal construct"
