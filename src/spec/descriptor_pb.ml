...
let default_enum_descriptor_proto_enum_reserved_range_mutable () : enum_descriptor_proto_enum_reserved_range_mutable = {
  start = None;
  end_ = None;
}
...
type enum_descriptor_proto_mutable = {
  ...
  mutable reserved_range : Descriptor_types.enum_descriptor_proto_enum_reserved_range list;
...
let default_method_options_mutable () : method_options_mutable = {
  deprecated = Some (false);
  idempotency_level = Some (Descriptor_types.default_method_options_idempotency_level ());
  uninterpreted_option = [];
}
...
let rec decode_enum_descriptor_proto_enum_reserved_range d =
  ...
  while !continue__ do
    ...
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(enum_descriptor_proto_enum_reserved_range), field(1)" pk
    | Some (2, Pbrt.Varint) -> begin
      v.end_ <- Some (Pbrt.Decoder.int_as_varint d);
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(enum_descriptor_proto_enum_reserved_range), field(2)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({
...
let rec decode_enum_descriptor_proto d =
  ...
  while !continue__ do
    ...
    | Some (4, Pbrt.Bytes) -> begin
      v.reserved_range <- (decode_enum_descriptor_proto_enum_reserved_range (Pbrt.Decoder.nested d)) :: v.reserved_range;
    end
    | Some (4, pk) -> 
...
let rec decode_descriptor_proto_extension_range d =
  ...
  while !continue__ do
    ...
    | Some (3, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(descriptor_proto_extension_range), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({
    Descriptor_types.start = v.start;
...
let rec decode_method_options_idempotency_level d = 
  match Pbrt.Decoder.int_as_varint d with
  | 0 -> (Descriptor_types.Idempotency_unknown:Descriptor_types.method_options_idempotency_level)
  | 1 -> (Descriptor_types.No_side_effects:Descriptor_types.method_options_idempotency_level)
  | 2 -> (Descriptor_types.Idempotent:Descriptor_types.method_options_idempotency_level)
...
let rec decode_file_options d =
  ...
  while !continue__ do
    ...
    | Some (42, Pbrt.Varint) -> begin
      v.php_generic_services <- Some (Pbrt.Decoder.bool d);
    end
    | Some (42, pk) -> 
...
let rec encode_enum_descriptor_proto_enum_reserved_range (v:Descriptor_types.enum_descriptor_proto_enum_reserved_range) encoder = 
...
let rec encode_extension_range_options (v:Descriptor_types.extension_range_options) encoder = 
  List.iter (fun x -> 
    Pbrt.Encoder.key (999, Pbrt.Bytes) encoder; 
    Pbrt.Encoder.nested (encode_uninterpreted_option x) encoder;
  ) v.Descriptor_types.uninterpreted_option;
...
let rec encode_method_options_idempotency_level (v:Descriptor_types.method_options_idempotency_level) encoder =
  match v with
...
let rec encode_method_options (v:Descriptor_types.method_options) encoder = 
  ...
  begin match v.Descriptor_types.idempotency_level with
  | Some x -> 
...
let rec encode_file_options (v:Descriptor_types.file_options) encoder = 
  ...
  begin match v.Descriptor_types.php_generic_services with
  | Some x -> 
  ...
  begin match v.Descriptor_types.swift_prefix with
  | Some x -> 
    Pbrt.Encoder.key (39, Pbrt.Bytes) encoder; 
    Pbrt.Encoder.string x encoder;
  | None -> ();
  end;
  begin match v.Descriptor_types.php_class_prefix with
  | Some x -> 
    Pbrt.Encoder.key (40, Pbrt.Bytes) encoder; 
    Pbrt.Encoder.string x encoder;
  | None -> ();
  end;
  begin match v.Descriptor_types.php_namespace with
  | Some x -> 
    Pbrt.Encoder.key (41, Pbrt.Bytes) encoder; 
    Pbrt.Encoder.string x encoder;
  | None -> ();
  end;
  begin match v.Descriptor_types.php_metadata_namespace with
  | Some x -> 
    Pbrt.Encoder.key (44, Pbrt.Bytes) encoder; 
    Pbrt.Encoder.string x encoder;
  | None -> ();
  end;
  begin match v.Descriptor_types.ruby_package with
  | Some x -> 
