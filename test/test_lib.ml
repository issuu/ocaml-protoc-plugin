...
module type T = sig
  ...
  val to_proto : t -> Protobuf.Protobuffer.t
  val from_proto : Protobuf.Protobuffer.t -> (t, Protobuf.Deserialize.error) result
  val name : string
end
...
let test_encode protobuf_file (type t) (module M : T with type t = t) (expect : t) =
  ...
  Out_channel.output_string cout (Protobuf.Protobuffer.contents data);
  Out_channel.close_no_err cout;
  Sys.command_exn (sprintf "protoc --decode=%s %s < %s | tr \"\\n\" \"; \"" M.name protobuf_file filename);
  Sys.remove filename;
  (* Decode the message *)
  let in_data = Protobuf.Protobuffer.reset data in
